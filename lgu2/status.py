...
def _outstanding_block(
    target: str, outstanding, changes_link: Optional[Link]
) -> Tuple[Message, SidePanel]:
    ...
    side_panel = SidePanel(
        ...
        paragraphs=(
            ...
            _(
                "This legislation has been amended but we still need to update the website to reflect these changes."
            ),
        ),
...
def _finalize(
    messages: List[Message], side_panel: Optional[SidePanel]
) -> Optional[Status]:
...
def for_document(meta) -> Optional[Status]:
    ...
    if is_most_recent_version(meta):
        ...
        outstanding = [
            e for e in (meta.get("unappliedEffects") or []) if e.get("outstanding")
        ]
        if outstanding:
