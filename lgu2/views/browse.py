...
def _group_counts_for_timeline(yearly_counts, complete_cutoff: Optional[int]):
    ...
    for count in yearly_counts:
        count["complete"] = (
            False if complete_cutoff is None else count["year"] >= complete_cutoff
        )
    ...
    for count in yearly_counts:
        ...
        if decade != last_decade:
            ...
            group = {
                "first_year": first_year_of_decade,
                "last_year": last_year_of_decade,
                "counts": [],
            }
...
def browse(request, type, year=None):
    ...
    for by_type in data["meta"]["counts"]["byType"]:
        ...
        by_type["link"] = (
            reverse("browse", args=[short_type])
            if year is None
    ...
    grouped_yearly_counts = zip_longest(
        *(iter(yearly_counts),) * 24
    )  # for yearPagination

    grouped_by_decade = _group_counts_for_timeline(yearly_counts, complete_cutoff)
    timeline_style = '<style type="text/css">#timeline #timelineData {{ width: {w}em }}</style>'.format(
        w=str(len(grouped_by_decade) * 35)
    )
    ...
    if "bySubjectInitial" in data["meta"]["counts"]:  # should not be present and None
        subject_initials = set(
            [i["initial"] for i in data["meta"]["counts"]["bySubjectInitial"]]
        )
    ...
    context = {
        ...
        "last_page": last_page,
        "search_endpoint": (
            reverse("browse", args=[type])
            if year is None
            else reverse("browse-year", args=[type, year])
        ),
