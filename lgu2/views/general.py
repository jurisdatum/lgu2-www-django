...
from lgu2.util.labels import (
    get_singular_type_label,
    get_long_type_label,
    get_type_label,
)
...
def homepage(request):
    ...
    context = {"new_legislation": data["documents"][:5]}
...
def different_legislature_by_country(request, country):
    ...
    return render(
        request,
        "new_theme/explore_collection/different_country_legislature.html",
        result,
    )
...
def legislature_eu_exit_uk_law(request):
    return render(
        request, "new_theme/explore_collection/explore-eu-exit-and-uk-law.html"
    )
...
def legislature_eu(request):
    return render(
        request,
        "new_theme/explore_collection/explore-legislation-originating-from-the-eu.html",
    )
...
def revised_legislation(request):
    ...
    return render(
        request, "new_theme/help_guide/revised-legislation.html", {"data": api_data}
    )
...
def secondary_legislation(request):
    ...
    api_data = get_wagtail_content(url)
    return render(
        request, "new_theme/help_guide/secondary-legislation.html", {"data": api_data}
    )
...
def group_documents_for_new_legislation_page(docs):
    ...
    return [
        {"longType": lt, "label": get_type_label(lt), "documents": group_docs}
        for lt, group_docs in ordered
    ]


def new_legislation(request, country=None, date=None):
    dates = local_cache.get_or_set(
        "recently_published_dates", get_recently_published_dates, 3600
    )
...
def health_dependencies(request):
    try:
        ...
        if response.status_code == 200:
            return JsonResponse({"status": "ok", "checks": {"api": "ok"}}, status=200)

        # Spring responded but unhealthy
        return JsonResponse(
            {"status": "unhealthy", "checks": {"api": "unhealthy"}}, status=503
        )

    except Exception:
        # Spring is unreachable / timeout / network error
        return JsonResponse(
            {"status": "unhealthy", "checks": {"api": "down"}}, status=503
        )
