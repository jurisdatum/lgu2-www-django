...
SORT_FIELDS = {
    "affected-title",
    "affected-year-number",
    "affecting-title",
    "affecting-year-number",
}
...
def _capture_query(
    type1=None, year1=None, number1=None, type2=None, year2=None, number2=None
):
...
def _make_api_parameters(
    query, page, title_params=None, applied=None, sort=None, orderBy=None
):
    params = {
    ...
    _add_year_params(
        params, query, "affected", "targetYear", "targetStartYear", "targetEndYear"
    )
    _add_year_params(
        params, query, "affecting", "sourceYear", "sourceStartYear", "sourceEndYear"
    )
...
def _make_nav(meta: Metadata, link_prefix: str, extra_params=None):
    ...
    pages = [
        ...
        for p in range(1 if page < 10 else page - 9, min(last_page, page + 9) + 1)
    ]
    ...
    return {
        ...
        "next": (
            {"number": page + 1, "link": make_link(page + 1)}
            if page < last_page
            else None
        ),
        "last": (
            {"number": last_page, "link": make_link(last_page)}
            if page < last_page
            else None
        ),
    }
...
def affected(request, applied=None, type=None, year=None, number=None, format=None):
    ...
    link_prefix = _build_link_prefix(
        viewname, applied=applied, type=type, year=year, number=number, format=format
    )
...
def _combined(request, query, link_prefix, format, applied):
    ...
    form_values = {
        **{
            f"{side}_{attr}": (
                query.get(f"{side}_{attr}") or ("all" if attr == "type" else "")
            )
    ...
    return HttpResponse(
        loader.get_template("new_theme/research-tools/changes-result.html").render(
            context, request
        )
    )
...
def _data(api_params, format):
    if format == "feed":
        return HttpResponse(
            api.fetch_atom(**api_params), content_type="application/atom+xml"
        )
