...
def build_changes_summary(form_values: dict, types: dict) -> ChangesSummary:
    ...
    affected_type_label = types.get(
        form_values.get("affected_type", ""), "any legislation"
    )
    affecting_type_label = types.get(
        form_values.get("affecting_type", ""), "any legislation"
    )
    ...
    clauses.extend(
        _side_clauses(form_values, "affected", affected_type_label, affected_year)
    )
    clauses.append(SummaryClause("made_by", ", made by"))
    clauses.extend(
        _side_clauses(form_values, "affecting", affecting_type_label, affecting_year)
    )

    form_initial = {
        "showChanges": f"{label} {connector}",
        **_side_form_initial(
            form_values, "affected", affected_type_label, affected_year
        ),
        **_side_form_initial(
            form_values, "affecting", affecting_type_label, affecting_year
        ),
    }
...
def _side_clauses(
    form_values: dict, prefix: str, type_label: str, year: str
) -> list[SummaryClause]:
    ...
    if year:
        clauses.append(SummaryClause(f"{prefix}_year", f", enacted or made {year}"))
