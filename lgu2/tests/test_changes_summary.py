...
class DetailSummaryTests(SimpleTestCase):
    ...
    def _detail_text(self, form_values):
        ...
        return (
            summary.detail_intro + "".join(c.text for c in summary.detail_clauses) + "."
        )

    def test_minimal_query(self):
        text = self._detail_text(_empty_form_values())
        self.assertEqual(
            text, "for All changes to any legislation, made by any legislation."
        )
    ...
    def test_year_range(self):
        text = self._detail_text(
            _empty_form_values(
                affected_type="uksi",
                affected_start_year="1996",
                affected_end_year="1998",
            )
        )
        self.assertEqual(
            text,
    ...
    def test_same_start_and_end_year(self):
        text = self._detail_text(
            _empty_form_values(
                affected_type="uksi",
                affected_start_year="2020",
                affected_end_year="2020",
            )
        )
