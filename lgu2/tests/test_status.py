...
class ForDocumentTests(SimpleTestCase):
    ...
    def test_returns_status_when_viewing_enacted_version(self):
        status = for_document(
            self._meta(version="enacted", versions=["enacted", "2024-06-01"])
        )
    ...
    def test_returns_side_panel_when_dated_version_is_latest(self):
        status = for_document(
            self._meta(version="2024-06-01", versions=["enacted", "2024-06-01"])
        )
        self.assertIsNotNone(status)
                        ...
                        "rich": [
                            {
                                "type": "link",
                                "text": "s. 5(2)(a)",
                                "href": "uksi/2025/349/section/5/2/a",
                            },
                        ],
                    },
                },
                        ...
                        "rich": [
                            {
                                "type": "link",
                                "text": "reg. 8(9)",
                                "href": "uksi/2019/707/regulation/8/9",
                            },
                        ],
                    },
                },
...
class ForFragmentTests(SimpleTestCase):
    ...
    def _meta(self, version, versions, *, fragment_label="Section 5", unapplied=None):
        return {
            ...
            "unappliedEffects": (
                unapplied if unapplied is not None else {"fragment": [], "ancestor": []}
            ),
        }
    ...
    def test_side_panel_targets_fragment_of_title_when_up_to_date(self):
        ...
        self.assertIsNotNone(status.side_panel)
        self.assertIn(
            "Section 5 of Test Act 2024", str(status.side_panel.paragraphs[0])
        )
    ...
    def test_outstanding_concatenates_direct_and_ancestor_in_order(self):
        ...
        status = for_fragment(
            self._meta(
                ...
                versions=["enacted", "2024-06-01"],
                unapplied={
                    "fragment": [direct, non_outstanding_direct],
                    "ancestor": [ancestor],
                },
            )
        )
...
class StatusMainTemplateTests(SimpleTestCase):
    ...
    def test_renders_one_panel_per_message(self):
        out = _render(
            ...
            {
                "status": Status(
                    messages=(Message(text="First."), Message(text="Second."))
                ),
            },
        )
    ...
    def test_renders_heading_when_present(self):
        out = _render(
            ...
            {
                "status": Status(
                    messages=(Message(heading="Heads up.", text="Body."),)
                ),
            },
        )
    ...
    def test_warning_severity_maps_to_not_up_to_date_class(self):
        out = _render(
            ...
            {
                "status": Status(
                    messages=(Message(text="Hello.", severity="warning"),)
                ),
            },
        )
    ...
    def test_renders_up_to_date_variant(self):
        out = self._render_with(
            SidePanel(
                ...
                links=(
                    Link(text="See all changes", href="/changes/affected/ukpga/2024/1"),
                ),
            )
        )
...
class DisclosureRenderingTests(SimpleTestCase):
    ...
    def _render_with(self, disclosure):
        return _render(
            ...
            {
                "status": Status(
                    messages=(Message(heading="H", text="T", disclosure=disclosure),)
                ),
            },
        )
