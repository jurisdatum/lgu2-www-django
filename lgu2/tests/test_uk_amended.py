...
class TestBuildBrowseUrlUkAmended(TestCase):
    ...
    def test_eu_type_with_year_and_uk_amended(self):
        result = build_browse_url_if_possible(
            {"type": "eur", "year": 2024, "ukAmended": False}
        )
...
class TestTypeFilterGroups(TestCase):
    ...
    @patch("lgu2.views.search.basic_search")
    def test_group_collapses_to_matching_sub_entry_when_filtered(
...
class TestActiveFiltersChipModel(TestCase):
    @patch("lgu2.views.search.basic_search")
    def test_uk_amended_chip_built_from_query_params_not_meta_echo(
        self, mock_basic_search
    ):
    ...
    @patch("lgu2.views.search.basic_search")
    def test_uk_amended_chip_remove_link_clears_only_that_param(
        self, mock_basic_search
    ):
        mock_basic_search.return_value = _eu_search_response(query={"type": "eur"})
