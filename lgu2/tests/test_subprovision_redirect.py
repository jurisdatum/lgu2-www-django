...
class SubprovisionRedirectTests(SimpleTestCase):
    ...
    def test_subprovision_data_urls_resolve_to_fragment_data_view(self):
        cases = [
            ...
            (
                "/ukpga/2024/1/section/1-2/2025-01-01/data.json",
                "fragment-version-data",
                {"version": "2025-01-01"},
            ),
            (
                "/ukpga/2024/1/section/1-2/english/data.json",
                "fragment-lang-data",
                {"lang": "english"},
            ),
            (
                "/ukpga/2024/1/section/1-2/2025-01-01/english/data.json",
                "fragment-version-lang-data",
    ...
    def test_section_subprovision_redirects_to_parent_with_anchor(
    ...
    @patch("lgu2.views.fragment.api.head")
    @patch("lgu2.views.fragment.api.get")
    def test_multi_level_subprovision_keeps_hyphenated_tail_in_anchor(
        self, mock_get, mock_head
    ):
        mock_head.return_value = 200

        response = self.client.get(
            reverse("fragment", args=["ukpga", 2024, 1, "section/1-2-a-i"])
        )

        self.assertEqual(response.status_code, 302)
        self.assertEqual(
            response["Location"],
            reverse("fragment", args=["ukpga", 2024, 1, "section/1"])
            + "#section-1-2-a-i",
        )
    ...
    def test_nested_schedule_paragraph_subprovision(self, mock_get, mock_head):
        ...
        response = self.client.get(
            reverse("fragment", args=["ukpga", 2024, 1, "schedule/1/paragraph/3-1"])
        )
    ...
    def test_article_kind_with_letter_in_parent(self, mock_get, mock_head):
        ...
        response = self.client.get(
            reverse("fragment", args=["uksi", 2024, 1, "article/1A-3"])
        )
    ...
    def test_preserves_version_segment(self, mock_get, mock_head):
        ...
        self.assertEqual(
            response["Location"],
            reverse(
                "fragment-version", args=["ukpga", 2024, 1, "section/1", "2025-01-01"]
            )
            + "#section-1-2",
        )
    ...
    def test_preserves_language_segment(self, mock_get, mock_head):
        ...
        response = self.client.get(
            reverse("fragment-lang", args=["ukpga", 2024, 1, "section/1-2", "english"])
        )

        self.assertEqual(response.status_code, 302)
        self.assertEqual(
            response["Location"],
            reverse("fragment-lang", args=["ukpga", 2024, 1, "section/1", "english"])
            + "#section-1-2",
        )
    ...
    def test_preserves_version_and_language(self, mock_get, mock_head):
        ...
        self.assertEqual(
            ...
            reverse(
                "fragment-version-lang",
                args=["ukpga", 2024, 1, "section/1", "2025-01-01", "english"],
            )
            + "#section-1-2",
        )
    ...
    def test_head_404_returns_404(self, mock_get, mock_head):
        ...
        response = self.client.get(
            reverse("fragment", args=["ukpga", 2024, 1, "section/1-999"])
        )
    ...
    def test_crossheading_with_hyphenated_title_is_excluded(self, mock_get, mock_head):
        ...
        self.client.get(
            reverse(
                "fragment",
                args=["ukpga", 2024, 1, "section/1/crossheading/something-or-other"],
            )
        )
    ...
    def test_each_subprovision_kind_is_detected(self, mock_get, mock_head):
        ...
        for kind in ("section", "article", "paragraph", "rule", "regulation"):
            with self.subTest(kind=kind):
                response = self.client.get(
                    reverse("fragment", args=["ukpga", 2024, 1, f"{kind}/1-2"])
                )
                self.assertEqual(response.status_code, 302)
