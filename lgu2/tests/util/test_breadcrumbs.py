...
class MakeBreadcrumbsTests(SimpleTestCase):
    ...
    def test_with_version_uses_versioned_toc_link(self):
        ...
        self.assertEqual(
            crumbs[2]["link"],
            reverse("toc-version", args=["ukpga", 2023, 1, "2024-01-01"]),
        )

    def test_with_lang_uses_lang_toc_link(self):
        crumbs = make_breadcrumbs(_meta("ukpga"), version=None, lang="welsh")
        self.assertEqual(
            crumbs[2]["link"], reverse("toc-lang", args=["ukpga", 2023, 1, "welsh"])
        )
    ...
    def test_first_two_crumbs_unchanged_by_has_toc(self):
        with_toc = make_breadcrumbs(
            _meta("ukpga"), version=None, lang=None, has_toc=True
        )
        without_toc = make_breadcrumbs(
            _meta("ukpga"), version=None, lang=None, has_toc=False
        )
