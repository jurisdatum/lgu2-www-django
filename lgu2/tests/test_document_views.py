...
class DocumentViewTests(SimpleTestCase):
    ...
    def test_document_view_returns_redirect_response(
        self, mock_get_document, mock_should_redirect
    ):
        expected_location = reverse(
            "document-version", args=["ukpga", 2024, 1, "2024-01-01"]
        )
    ...
    @patch("lgu2.views.document.get_document")
    def test_ukia_document_page_breadcrumbs_link_to_clean_results_pages(
        ...
        self.assertContains(
            response, f'href="{reverse("browse-year", args=["ukia", 2024])}"'
        )

    @patch("lgu2.views.document.get_document")
    def test_document_page_renders_related_info_when_associated_present(
        self, mock_get_document
    ):
        mock_get_document.return_value = _document_response(
            associated=[
        ...
        response = self.client.get(
            reverse("document-version", args=["ukpga", 2024, 1, "enacted"])
        )
    ...
    @patch("lgu2.views.document.get_document")
    def test_document_page_omits_related_info_sections_when_associated_empty(
        self, mock_get_document
    ):
...
class TocViewTests(SimpleTestCase):
    ...
    def test_toc_pdf_only_enacted_shows_kings_printer_message(self, mock_get_toc):
        ...
        response = self.client.get(
            reverse("toc-version", args=["ukpga", 2024, 1, "enacted"])
        )
    ...
    def test_toc_pdf_only_revised_omits_kings_printer_message(self, mock_get_toc):
        ...
        response = self.client.get(
            reverse("toc-version", args=["ukpga", 2024, 1, "2024-06-01"])
        )
