...
class ChangesRedirectTests(SimpleTestCase):
    ...
    def test_changes_intro_redirect_prefers_selected_range_over_stale_specific_year(
        self,
    ):
        response = self.client.get(
            reverse("changes-intro"),
    ...
    def test_changes_intro_redirect_preserves_affecting_filters_in_combined_searches(
        self,
    ):
        response = self.client.get(
            reverse("changes-intro"),
...
class ChangesResultsFeedTests(SimpleTestCase):
    ...
    def test_feed_accepts_year_range_and_applied_filters(self, mock_get):
        ...
        queries = [
            parse_qs(urlsplit(call.args[0]).query) for call in mock_get.call_args_list
        ]
        ...
        self.assertTrue(
            all(
                call.args[1] == "application/atom+xml"
