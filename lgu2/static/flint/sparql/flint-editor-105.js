...
function FlintEditor(container, imagesPath, config) {   
	...
	function FlintResults(editor) {
		...
		this.setResults = function(text) {
			...
			if (resultsMode === "Visual"
				...
				var resultsOb= $($.parseXML(results));
				// Collect variables (column headings)
				var devUsedVariables = new Array();
				resultsOb.find("variable").each(function() {
					var columnName = $(this).attr("name");
					headTr.append($("<th>").text(columnName));
					devUsedVariables.push(columnName);
				});
				var nVars= devUsedVariables.length;
				resultsOb.find("result").each(
					function() {
						var resultTr= $("<tr>");
						var resultItem = $(this);
						// Loop over variables (i.e. column headings)
						for(var i=0; i<nVars; i++) {
							var varname= devUsedVariables[i];
							var bindings= resultItem.find("binding");
							var foundVarBinding= false;
							// Loop over bindings and look for match
							// (Note that direct resultItem.find("binding[name='bla']")
							//  didn't work in IE8)
							bindings.each( function() {
								if ($(this).attr('name')==varname) {
									foundVarBinding= true;
									$(this).children().first().each( function() {
										var text=$(this).text().replace(/\"/g,"\\\"");
										var resultTd=$("<td>");
										var tagname=this.tagName.toUpperCase();
										if (tagname=="URI"||(tagname=="BNODE"&&text.startsWith("nodeID://"))) {
											// Resource URI or weird virtuoso bnode
											resultTd.append($("<span class='sp-uri'>").text("<"+text+">"));
										} else if (tagname=="BNODE") {
											if(!text.startsWith("_:")) text="_:"+text;
											resultTd.append($("<span class='sp-prefixed'>").text(text));
										} else {
											var datatype=$(this).attr("datatype");
											var lang=$(this).attr("xml:lang");
											if(lang!=null) {
												resultTd
													.append($("<span class='sp-literal'>").text('"'+text+'"'))
													.append($("<span class='sp-punc'>").text("@"+lang));
											} else if(datatype==null) {
												resultTd.append($("<span class='sp-literal'>").text('"'+text+'"'));
											} else if(datatype=="http://www.w3.org/2001/XMLSchema#boolean") {
												resultTd.append($("<span class='sp-keyword'>").text(text));
											} else if(/^http:\/\/www.w3.org\/2001\/XMLSchema#((integer)|(decimal)|(double))$/.test(datatype)) {
												resultTd.append($("<span class='sp-literal'>").text(text));
											} else if(/^http:\/\/www.w3.org\/2001\/XMLSchema#/.test(datatype)) {
												resultTd
													.append($("<span class='sp-literal'>").text('"'+text+'"'))
													.append($("<span class='sp-punc'>").text("^^"))
													.append($("<span class='sp-prefixed'>").text("xsd:"+datatype.replace('http://www.w3.org/2001/XMLSchema#','')));
											} else {
												resultTd
													.append($("<span class='sp-literal'>").text('"'+text+'"'))
													.append($("<span class='sp-punc'>").text("^^"))
													.append($("<span class='sp-uri'>").text("<"+datatype+">"));
											}
										}
										resultTr.append(resultTd);
									});
								}
								return !foundVarBinding;
							});
							// No match - blank cell in table
							if (!foundVarBinding) {
								resultTr.append($("<td>").text("-"));
							}
						}
						resultTbody.append(resultTr);
					});                                                
				var resultTable= $("<table id='flint-results-table'>")
					.append($("<thead>").append(headTr))
					.append(resultTbody);
				try {                              
					$('#flint-results-container').empty().append(resultTable);
                } catch (e) {
					editor.getErrorBox().show(e);
                 }
			} else {
				try { 
					$('#flint-results-container').html('<textarea id="flint-results"></textarea>');
					$('#flint-results-container > textarea').text(results);
				} catch (e) {
					editor.getErrorBox().show(e);
				}
			}
	...
	function FlintEndpointEntry(config, editor) {
		...
		try {
			...
			this.display = function(container) {
				// Set default endpoint (can be changed in browser)
				var endpoint = config.endpoints[0].uri;
				$('#' + container)
						.append(
	...
	// Pick Display or Open (download) as result action
	function FlintResultActionPicker(config, editor) {
		try {
		
			this.display = function(container) {
				var listItems = "";
				listItems += "<option value='Display'>Display results</option>";
				listItems += "<option value='Open'>Open results</option>";
				$('#' + container)
					.append(
					"<div id='flint-result-action' title='Select action for query results'><h2 id='flint-result-action-label'>Action</h2><select id='flint-result-action-select' name='result-action'>" + listItems + "</select></div>");
			};

			this.getResultAction = function() {
				return $("#flint-result-action-select").val();
			};

			this.setChangeAction = function(callback) {
				$('#flint-result-action-select').change(callback);
			};
	...
	function FlintEndpointQuerySubmitButton(editor) {
		...
		this.setSubmitAction = function(callback) {
			$('#flint-endpoint-submit').submit(callback);
		};
	}
	...
	function FlintCoolbar(config, editor) {
		...
		try {
			...
			coolbarItems.push(new FlintResultActionPicker(config, editor));
			this.getItems = function() {
				return coolbarItems;
			};
		} catch (e) {
	...
	function FlintSidebar(editor, config) {
			...
			try {
				...
				if (config.namespaces !== null) {
					...
					for (j = 0; j < config.namespaces.length; j++) {
						...
						var prefix;
						var i;
						if (config.namespaces[j].forceInclude ||
							activeDataItem.properties === null) {
								...
								.click(
										function(e) {
											...
											if (editor.getCodeEditor().getValue() != "") {                                                                                               
                                                                                                if(typeof config.viewMode === 'undefined' || typeof config.viewMode.windowCloseConfirm === 'undefined' || config.viewMode.windowCloseConfirm) {
                                                                                                   editor.getConfirmDialog().setCloseAction(function() {
		...
		function showTab(tabName, id) {
			...
			if (tabName == "Properties") {
				...
				displaySamples();
			} else if (tabName === "SPARQL-Select") {
				displaySparql();
			} else {
                            var defaultTab;
                            if(typeof config.viewMode !== 'undefined' && typeof config.viewMode.defaultTab !== 'undefined'){
                                defaultTab = config.viewMode.defaultTab;
                            } else {
                                defaultTab = null;
                            }
                            switch(defaultTab){
                                case 'prefixes':          
                                    ...
                                    $('#flint-sidebar-prefixes').addClass("flint-sidebar-selected");
                                    break;
                                default:                                    
		...
		this.display = function(container) {
			...
			$('#flint-sidebar-sparql').click(function(e) {
				showTab("SPARQL-Select", $(this).attr("id"));
				e.stopPropagation();
			});
	...
	function FlintCodeEditor(flint, editorMode) {
		...
		function getCompletions(token, start, possibles) {
			...
			if (lastProperty
					...
					&& (topSymbol == "object" || topSymbol == "objectList" || topSymbol == "objectListPath"	|| topSymbol == "}" || topSymbol == "]")) {
				isClassPos = true;
			}
	...
	function Flint(container, imagesPath, config) {
		...
		try {
			...
			// Get a handle on the Result Action picker
			var resultActionPicker= createCoolbar.getItems()[4];
			...
			this.sendDatasetQuery = function(event) {
				var resultAction= resultActionPicker.getResultAction();
						...
						$.ajax({
							...
							error : function(XMLHttpRequest, textStatus, errorThrown) {
								if (XMLHttpRequest.status == 0) {
									...
									var text, textRaw= XMLHttpRequest.responseText;
									var contentType= XMLHttpRequest.getResponseHeader("Content-type");
									if(contentType==null) {
										text="";
									} else if (contentType.includes("text/plain")) { 
										var elem= $("<div>").append($("<pre>").text(textRaw));
										text="<br/>"+elem.html();
										elem.remove();
									} else if (/\/x?html/.test(contentType)) {
										var elem= $("<div>").append($("<iframe>").attr("srcdoc",textRaw));
										text="<br/>"+elem.html();
										elem.remove();
									} else {
										text="";
									...
									errorBox.show("Dataset Request: HTTP Status: "
											+ XMLHttpRequest.status + "; " + textStatus + "; "+ errorThrown + text);
								}
			...
			if (cm.getCodeMirror().getValue() != "" 
				&& (typeof config.viewMode === 'undefined' 
					|| typeof config.viewMode.windowCloseConfirm === 'undefined' 
					|| config.viewMode.windowCloseConfirm)) {
			...
			this.insert = function(text) {
				...
				cm.getCodeMirror().focus();
                        ...
                        var replacePrefixes = true;
                        
                        this.insertPrefix = function(text) {   
                                var editorContent = text + '\n' + cm.getCodeMirror().getValue();
                                cm.getCodeMirror().setValue(editorContent);
                                cm.getCodeMirror().focus();
			};
			...
			this.insertQuery = function(title, query, line, ch) {
				if(typeof config.viewMode === 'undefined' || typeof config.viewMode.addPrefixesToSampleQueries === 'undefined' || config.viewMode.addPrefixesToSampleQueries) {
					query= createSidebar.getPrefixes() + "\n" + query;
					line+= createSidebar.getPrefixCount();
				}
				if(typeof config.viewMode === 'undefined' || typeof config.viewMode.windowCloseConfirm === 'undefined' || config.viewMode.windowCloseConfirm) {
					confirmDialog.setCloseAction(function() {
						var result = confirmDialog.getResult();
							if (result == "Okay") {
								cm.getCodeMirror().setValue(query);
								cm.getCodeMirror().setCursor(line, ch);
								flint.formatQuery();
							cm.getCodeMirror().focus();
							}
					});
	...
	this.getEditor = function() {
        ...
        $('#flint-sidebar-grabber-button').click();
